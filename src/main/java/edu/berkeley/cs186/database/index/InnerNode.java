...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        int keyIndex = LeafNode.findIndexLessThan(key, this.keys);
        Optional<Pair<DataBox, Long>> pair = this.getChild(keyIndex).put(key, rid);
        if (pair.isPresent()) {
            DataBox pushKey = pair.get().getFirst();
            Long pageNum = pair.get().getSecond();
            int index = LeafNode.findIndexLessThan(key, this.keys);
            this.keys.add(index, pushKey);
            this.children.add(index + 1, pageNum);
            return this.insert(this.keys.size() - 1);
        }
    ...
    private Optional<Pair<DataBox, Long>> insert(int length) {
        Optional<Pair<DataBox, Long>> pair = Optional.empty();

        if (length >= this.metadata.getOrder() * 2) {
            DataBox returnKey = this.keys.get(length / 2);
            List<DataBox> newKeys = this.keys.subList(length / 2 + 1, this.keys.size());
            List<Long> newChildren = this.children.subList(length / 2 + 1, this.children.size());

            this.keys = this.keys.subList(0, length / 2);
            this.children = this.children.subList(0, length / 2 + 1);

            InnerNode rightInnerNode = new InnerNode(this.metadata, this.bufferManager,
                    newKeys, newChildren, this.treeContext);
            Optional<Long> newPageNum = Optional.of(rightInnerNode.getPage().getPageNum());
            pair = Optional.of(new Pair<DataBox, Long>(returnKey, newPageNum.get()));
        }
        this.sync();
        return pair;
    }
