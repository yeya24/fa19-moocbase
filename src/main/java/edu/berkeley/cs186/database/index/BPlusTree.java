...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        return new BPlusTreeIterator(this.root.get(new IntDataBox(key.getInt())), key.getInt());
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        pair.ifPresent(dataBoxLongPair -> this.insert(dataBoxLongPair.getFirst(), dataBoxLongPair.getSecond()));
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // If the tree is not empty, raise an exception.
        if (this.root.getLeftmostLeaf().getKeys().size() > 0) {
    ...
    private void insert(DataBox key, Long child) {
        List<DataBox> newKeys = new ArrayList<>();
        List<Long> newChildren = new ArrayList<>();
        newKeys.add(key);
        newChildren.add(this.root.getPage().getPageNum());
        newChildren.add(child);
        InnerNode innerNode = new InnerNode(this.metadata, this.bufferManager,
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode leafNode) {
            this.node = leafNode;
            this.ridIterator = leafNode.getRids().iterator();
        }

        public BPlusTreeIterator(LeafNode leafNode, int min) {
            this.node = leafNode;
            this.ridIterator = leafNode.getRids().iterator();
            this.firstIteration(min);
        }

        public void firstIteration(int min) {
           Iterator<DataBox> keyIterator = this.node.getKeys().iterator();
           while (keyIterator.hasNext()) {
               int key = keyIterator.next().getInt();
               if (key >= min) {
                   return;
               }
               this.ridIterator.next();
           }
        }

        @Override
        public boolean hasNext() {
            return this.ridIterator.hasNext() || this.node.getRightSibling().isPresent();
        }

        @Override
        public RecordId next() {
            if (this.ridIterator.hasNext()) {
                return this.ridIterator.next();
            }
            Optional<LeafNode> sibling = this.node.getRightSibling();
            if (sibling.isPresent()) {
                this.node = sibling.get();
                this.ridIterator = this.node.getRids().iterator();
            }
            return this.ridIterator.next();
        }
    }
